...
分岐側などを緩いS字として表示する場合も `bezier` を使用する。
1セグメントの3次ベジェ曲線として、2つの制御点を記述する。

`from` と `to` に対応するコネクタが部品定義に存在する場合、直線・カーブ・ベジェの始点と終点はコネクタの `position` から導出する。
その場合、最終セグメントの `endPosition` は省略でき、終点の x/y/z は `to.position` から導出する。
ベジェ制御点の z を省略した場合、始点側の制御点は始点 z、終点側の制御点は終点 z を使用する。
...
`connector` は、その切り替えポイントが対象とするコネクタのインデックス。
将来の切り替え判定やレイアウト上のポイント表示で使用する。
...
とする。通常のレールは z=0 のため高さは変化せず、坂レールでは flip によって高い側と低い側が入れ替わる。

反転後は、接続グループまたは単体レールの最低 z 座標を0に正規化する。
...
- `""`: 通常。進入コネクタを対象とするスイッチの状態に従う。選択した path を別のスイッチも対象としている場合は、そのスイッチも走行する path がアクティブになるよう更新する。分岐側から非アクティブな path へ進入した場合も同様に更新する。
- `"auto-switch"`: 通常と同じ path の選択・同期を行い、進入コネクタを対象とするスイッチは進入時に次の状態へ切り替える。
- `"fixed"`: 進入コネクタを対象とするスイッチの現在状態には従うが、走行する path を対象とする他のスイッチは更新しない。
